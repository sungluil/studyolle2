...
public class AccountController {
	...
	public String emailLoginForm(String email, Model model, RedirectAttributes attributes) {
		Account account = accountRepository.findByEmail(email);
		if(account == null) {
			model.addAttribute("message", "유효한 이메일이 아닙니다.");
			return "account/email-login";
		}
		
		if(!account.canSendConfirmEmail()) {
			model.addAttribute("error", "로그인 이메일은 1시간에 한번만 전송할 수 있습니다.");
//			return "account/email-login";
		}
		accountService.sendLoginLink(account);
		attributes.addFlashAttribute("message", "메일을 성공적으로 발송했습니다.");
		return "redirect:/email-login";
	}
	
	
	@GetMapping("/login-email-token")
	public String emailLoginToken(String token, String email, Model model) {
		Account account = accountRepository.findByEmail(email);
